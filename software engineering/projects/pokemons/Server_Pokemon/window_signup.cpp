...
Window_SignUp::Window_SignUp(QWidget *parent) :
    QDialog(parent),
...
void Window_SignUp::initSocket()
{
    ...
    connect(this->receiver,SIGNAL(readyRead()),this,SLOT(readPendingDatagram()));
    ...
    else
    {
        ...
        if(ui->password_Text->text()==ui->password_Again_Text->text())
        {
           ...
           // QString nameEncrypted=PokemonEncrypter::encryptItem(QByteArray(ui->userName_Text->text().toLatin1().data()));
            ...
            signUpInfo.setAddress(PokemonServer::DEFAULT_ADDRESS);
            signUpInfo.setPort(PokemonServer::DEFAULT_PORT);
...
//        signInInfo.push_back(passwordEncrypted);
        ...
        signInfo.setAddress(PokemonServer::DEFAULT_ADDRESS);
        signInfo.setPort(PokemonServer::DEFAULT_PORT);
...
{
   // dataProcessor->setWindow(this);
   // dataProcessor->receiveDataClient(receiver,currentPlayer);
}
